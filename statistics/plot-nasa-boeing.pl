...
my @tools = ("nuXmv-bmc", "nuXmv-klive", "z3-1000", "z3-10000", "z3-100000");
my $output_encoding = "./encoding-nasa-boeing.csv";
my $output_solving = "./solving-nasa-boeing.csv";
my $output_total = "./total-nasa-boeing.csv";
...
open (TOTAL_OUT, ">${output_total}") or die "cannot open ${output_total}\n";
    ...
    for (my $i = 0; $i < @tools; ++ $i) {
        ...
        if ($line_res eq "") {
            ...
            if ($data[0] eq "unknow") {
            	...
            	$total_cost = $solving[$i] + $timeout;
            }
            else {
            	$solving_cost = $solving[$i] + $data[2];  
            	$total_cost = $solving[$i] + $data[2];  
            } 
        }
